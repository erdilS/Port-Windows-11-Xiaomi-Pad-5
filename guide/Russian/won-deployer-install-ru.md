...
---
## 3. Настройка двойной загрузки

### Требования
- ```Подключение к интернету на устройстве Nabu (требуется для загрузки файлов!)```

### Настройка - Android
- После завершения настройки Android перезагрузите ваш планшет(Nаbu).
- Откройте предустановленное приложение `Won-deployer Setup`.
- Последовательно нажмите на каждую кнопку и следуйте инструкции на экране.
- Теперь откройте приложение `Magisk`(если не открылось автоматически), нажмите `OK`, чтобы завершить дополнительную настройку и выполните её.
- После перезагрузки откройте `Magisk` заново. Если оно снова запросить дополнительную установку, нажмите `OK` и перезагрузитесь снова.
- После финальной перезагрузки откройте приложение `WOA Helper` и предоставьте ему root-доступ.
