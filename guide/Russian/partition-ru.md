...
> Чтобы установить пользовательские размеры, выполните ``раздел оболочки adb [ЦЕЛЕВОЙ РАЗМЕР WINDOWS В ГБ]``

> Убедитесь, что вы не добавляете ГБ в конце, только количество
```cmd
adb shell partition
```
